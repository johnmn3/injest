...
(deftest lookup-key-by-string-in-map
  (testing "Get value by string key"
    (is (= 5 (p/+> {0 :a "s" 5} "s")))
    (is (= 5 (p/+>> {0 :a "s" 5} "s")))
    (is (= 5 (p/x> {0 :a "s" 5} "s")))
    (is (= 5 (p/x>> {0 :a "s" 5} "s")))
...
(deftest lookup-key-by-key-in-map
  (testing "Get value by keyword key"
    (is (= 5 (p/+> {0 :a :k 5} :k)))
    (is (= 5 (p/+>> {0 :a :k 5} :k)))
    (is (= 5 (p/x> {0 :a :k 5} :k)))
    (is (= 5 (p/x>> {0 :a :k 5} :k)))
...
(deftest lookup-key-by-boolean-in-map
  (testing "Get value by boolean key"
    (is (= 5 (p/+> {0 :a true 5} true)))
    (is (= 5 (p/+>> {0 :a true 5} true)))
    (is (= 5 (p/x> {0 :a true 5} true)))
    (is (= 5 (p/x>> {0 :a true 5} true)))
    (is (= 5 (p/=> {0 :a true 5} true)))
    (is (= 5 (p/=>> {0 :a true 5} true)))))
  ...
  (testing "test all the path features at once"
    (is (= "hi bob!"
           (let [m {1 (rest ['ignore0 0 1 {"b" [0 1 {:c {true {nil :bob}}}]}])}]
             (p/+> m 1 2 "b" 2 :c true nil name #(str "hi " % "!")))))
    (is (= "hi bob!"
           (let [m {1 (rest ['ignore0 0 1 {"b" [0 1 {:c {true {nil :bob}}}]}])}]
             (p/+>> m 1 2 "b" 2 :c true nil name #(str "hi " % "!")))))
    (is (= "hi bob!"
           (let [m {1 (rest ['ignore0 0 1 {"b" [0 1 {:c {true {nil :bob}}}]}])}]
             (p/x> m 1 2 "b" 2 :c true nil name #(str "hi " % "!")))))
    (is (= "hi bob!"
           (let [m {1 (rest ['ignore0 0 1 {"b" [0 1 {:c {true {nil :bob}}}]}])}]
             (p/x>> m 1 2 "b" 2 :c true nil name #(str "hi " % "!")))))
    (is (= "hi bob!"
...
(deftest thread-first-transducers
  ...
  (testing "x> with map and filter"
    (is (= [2 4] (vec (p/x> [1 2 3 4] (map inc) (filter even?))))))

  (testing "x> with transducer and vec"
    (is (= [2 4] (p/x> [1 2 3 4] (map inc) (filter even?) vec))))

  (testing "+> thread-first semantics with multi-arg fn"
    (is (= {:a 1 :b 2} (p/+> {:a 1} (assoc :b 2))))
    (is (= {:a 1 :b 2 :c 3} (p/+> {:a 1} (assoc :b 2) (assoc :c 3)))))

  (testing "+> with conj threads as first arg"
    (is (= [1 2 3] (p/+> [1 2] (conj 3))))))
...
(deftest thread-last-semantics
  (testing "+>> threads as last arg"
    (is (= [1 2 3 99] (p/+>> 99 (conj [1 2 3])))))

  (testing "+>> with range"
    (is (= '(0 1 2) (p/+>> 3 (range)))))

  (testing "+>> with str"
    (is (= "hello world" (p/+>> " world" (str "hello"))))))
...
(deftest pipeline-parallel-macros
  ...
  (testing "|>> with map and filter"
    (is (= [2 4] (vec (p/|>> [1 2 3 4] (map inc) (filter even?))))))
  ...
  (testing "navigating JSON-like nested data"
    (let [api-response {:status 200
                        :body {:users [{:name "Alice" :age 30}
                                       ...
                                       {:name "Carol" :age 35}]}}]
      (is (= "Bob"
             (p/+> api-response :body :users 1 :name)))
      (is (= "Bob"
             (p/+>> api-response :body :users 1 :name)))
      (is (= 35
             (p/+> api-response :body :users 2 :age))))))
  ...
  (testing "navigating config-like nested data"
    (let [config {:db {:host "localhost"
      ...
      (is (= 5432
             (p/+> config :db :port)))
      (is (= 10
             (p/+> config :db :pools "main" :size)))
      (is (= 20
             (p/+>> config :db :pools "read" :size))))))
...
(deftest transducer-pipeline-on-data
  ...
  (testing "filter and aggregate"
    (is (= [2 4 6 8 10]
           (vec (p/x>> (range 1 11) (filter even?))))))
